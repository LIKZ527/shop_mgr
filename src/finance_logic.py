...
from src.database_setup import get_conn
...
def reverse_split_on_refund(order_number: str):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            # 回冲各资金池
            cur.execute("SELECT pool_type,amount FROM order_split WHERE order_number=%s AND item_type='pool'", order_number)
            for row in cur.fetchall():
                # 这里演示：把池子金额扣减（真实场景可能记负流水）
                pass
            conn.commit()
...
def generate_statement():
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("INSERT INTO merchant_statement(merchant_id,date,opening_balance,income,withdraw,closing_balance) VALUES(%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE opening_balance=VALUES(opening_balance),income=VALUES(income),withdraw=VALUES(withdraw),closing_balance=VALUES(closing_balance)",
